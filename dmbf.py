...
def massal():
    ...
    if len(id) == 0:
       ...
       print(f"{P}[*]{M} Maaf total id anda adalah {B}{len(id)}");menu()
        ...
        print(f"{B} | ")
        print(f"{P}[*] Total id : {B}{len(id)}");open('dump/dump.json','r').read()
        fii_xd()
...
def followerss():
    ...
    if len(id) !=0:
        ...
        print(f"{P}[*] Total id : {B}{len(id)}");open('dump/dump.json','r').read()
        fii_xd()
    else:print(f"{P}[*] {M} total id : {M}{len(id)}");open('dump/dump.json','r').read()
...
def userset():
    ...
    if _pil_ in ["1","01"]:
        ...
        try:
            open("tool/ua.json","w").write(lololr)
            usera=open("tool/ua.json","r").read()
        ...
        try:
            _tes_ua=open("tool/ua.json","r").read()
        except IOError:
